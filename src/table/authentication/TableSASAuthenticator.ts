...
import { TableSASResourceType } from "./TableSASResourceType";
...
export default class TableSASAuthenticator implements IAuthenticator {
  ...
  public async validate(
    ...
    if (resource !== TableSASResourceType.Table) {
      this.logger.debug(
        // tslint:disable-next-line:max-line-length
        `TableSASAuthenticator:validate() Signed resource type ${resource} is invalid. Skip table service SAS validation.`,
        context.contextID
      );
      return undefined;
    }
    this.logger.debug(
      `TableSASAuthenticator:validate() Signed resource type is ${resource}.`,
      context.contextID
    );
    ...
    if (tableSASPermission === undefined) {
      throw new Error(
        // tslint:disable-next-line:max-line-length
        `TableSASAuthenticator:validate() ${
          resource === TableSASResourceType.Table
            ? "OPERATION_TABLE_SAS_TABLE_PERMISSIONS"
            : "OPERATION_TABLE_SAS_CONTAINER_PERMISSIONS"
